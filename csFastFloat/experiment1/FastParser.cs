...
namespace csFastFloat.experiment1
{
  ...
  public sealed class FastParser
  {
    ...
    unsafe static public double ParseDouble(char* first, char* last, chars_format expectedFormat = chars_format.is_general, char decimal_separator = '.')
      => ParseNumber(first, last,  new DoubleBinaryFormat2(), expectedFormat, decimal_separator);

 
    unsafe static internal Double ParseNumber (char* first, char* last, DoubleBinaryFormat2 binaryFormat, chars_format expectedFormat = chars_format.is_general, char decimal_separator = '.')
    {
      ...
      if (!pns.valid)
      {
        return HandleInvalidInput(first, last, binaryFormat);
      }
    ...
    internal static AdjustedMantissa ComputeFloat (long q, ulong w, DoubleBinaryFormat2 binaryFormat)
    {
    ...
    internal static AdjustedMantissa ComputeFloat(DecimalInfo d, DoubleBinaryFormat2 binaryFormat)
    {
    ...
    unsafe static internal AdjustedMantissa ParseLongMantissa(char* first, char* last, DoubleBinaryFormat2 binaryFormat, char decimal_separator)
    {
    ...
    unsafe static internal double HandleInvalidInput(char* first, char* last, DoubleBinaryFormat2 binaryFormat)
    {
      if (last - first >= 3)
      {
