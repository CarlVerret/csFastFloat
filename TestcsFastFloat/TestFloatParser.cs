...
namespace TestcsFastFloat.Tests
{
  public class TestFloatParser : BaseTestClass
  {
    ...
    //[InlineData("0e9999999999999999999999999999", 0.f)]
    ...
    //public private void VerifierCas_Float(string sut, float expected)
    //{
    //}
    ...
    //{
    //  for (int p = -127; p <= 308; p++)
    //  {
    //    if (p == 23)
    //      p++;

    //    var sut = new FloatParser();

    //    float? d = sut.ToFloat(false, sut.ComputeFloat(p, 1));
    ...
    //    {
    //      float? d = new FloatParser().ParseNumber(pstart, pstart + sut.Length, chars_format.is_general);
