  ...
  public class SupplFilesTest : BaseTestClass
  {
    ...
    private static void VerifyFile(string fileName)
    {
      ...
      while (!fs.EndOfStream)
      {
          ...
          Assert.True(_f == f);
          double d = FastDoubleParserSIMD2.ParseDouble(sut[3]);
          Assert.True(_d == d);

          // parse and assert equality
          float f_span = FastFloatParser.ParseFloat(sut[3].AsSpan());
          Assert.True(_f == f_span);
          double d_span = FastDoubleParserSIMD2.ParseDouble(sut[3].AsSpan());
          ...
          double d_utf8 = FastDoubleParserSIMD2.ParseDouble(System.Text.Encoding.UTF8.GetBytes(sut[3]));
