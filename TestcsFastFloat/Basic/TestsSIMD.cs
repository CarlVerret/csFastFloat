...
namespace TestcsFastFloat.Basic.SIMD
{
  public class TestsSIMD : BaseTestClass
  {
    ...
    [Fact]
    public void eval_and_parse_variable_digits_simd_works()
    {
      ...
      for (int i = 0; i <= 9; i++)
      {
        for (int j = 1; j != 9; j++)
        {
          ...
          string sut = new string(i.ToString()[0], j);
          unsafe
          {
            fixed (char* start = sut)
            {
    ...
    [Fact]
    public void eval_and_parse_variable_digits_simd_works_rnd()
    {
    ...
    [SkippableFact]
    unsafe public void ParseNumberVariable_String_Works_Scnenarios()
    {
      Skip.If(base.NoDiffToolDetected(), "No diff tool detected");
